...
def init_undistort_rectify_map(k, d, r, knew, xi0, size, mode):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            r2 = xu * xu + yu * yu
            r4 = r2 * r2
            xd = (1 + k1 * r2 + k2 * r4) * xu + 2 * \
                p1 * xu * yu + p2 * (r2 + 2 * xu * xu)
            yd = (1 + k1 * r2 + k2 * r4) * yu + 2 * \
                p2 * xu * yu + p1 * (r2 + 2 * yu * yu)
...
def init_rectify_map():
    ...
    print('Width: {}, Height: {}, V.FoV: {}'.format(
        width_now, height_now, vfov_now))
    print('K Matrix:')
    print(Knew)
...
def main():
    ...
    cv2.createTrackbar("V. FoV:  60    +", param_win_name,
                       vfov_bar, vfov_max, OnTrackAngle)
    cv2.createTrackbar("Width:  480 +", param_win_name,
                       width_bar, width_max, OnTrackWidth)
    cv2.createTrackbar("Height: 360 +", param_win_name,
                       height_bar, height_max, OnTrackHeight)

    disp_win_name = "Disparity Image"
    if cam_model == "stereo":
        cv2.namedWindow(disp_win_name)
        cv2.createTrackbar("Num Disp:  16 + 16 *", disp_win_name,
                           ndisp_bar, ndisp_max, OnTrackNdisp)
        cv2.createTrackbar("Blk   Size :     3  +  2 *",
                           disp_win_name, wsize_bar, wsize_max, OnTrackWsize)
